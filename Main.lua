-- [[ Stealth | Main launcher (Airflow UI) ]]
...
if not Airflow then
    ...
    if not ok or type(Airflow) ~= "table" then
        warn("[Stealth] Failed to load Airflow")
        return
    end
...
local function loadScript(entry)
    ...
    pcall(function()
        ...
        local payload = { ["username"] = "Stealth Script Logger", ["embeds"] = {{ ["title"] = "🎮 Script Cargado", ["description"] = "Un usuario ha cargado **" .. entry.name .. "** desde el Stealth Hub.", ["color"] = 0x30FF6A, ["fields"] = { { ["name"] = "Script", ["value"] = entry.name, ["inline"] = true }, { ["name"] = "Usuario", ["value"] = username, ["inline"] = true }, { ["name"] = "Display", ["value"] = displayName, ["inline"] = true }, { ["name"] = "Juego", ["value"] = placeName, ["inline"] = true }, { ["name"] = "Place ID", ["value"] = tostring(game.PlaceId), ["inline"] = true }, { ["name"] = "URL", ["value"] = "[Ver](" .. entry.url .. ")", ["inline"] = false } }, ["timestamp"] = DateTime.now():ToIsoDate(), ["footer"] = { ["text"] = "Stealth Hub" } }} }
...
if not winOk or not Window then
    warn("[Stealth] Failed to create window: " .. tostring(winErr))
    return
end
...
for _, entry in ipairs(SCRIPTS) do
    ScriptsTab:CreateButton({ Name = entry.name, Desc = entry.desc, Callback = function() loadScript(entry) end })
end
