	...
	if propertyTable then
		for Property, Value in propertyTable do
			...
			if not Success then
				...
				warn("error on line " .. tostring(Line) .. ": " .. tostring(Error))
			end
		end
	end
...
local function Overwrite(to_overwrite, overwrite_with, _depth)
	_depth = _depth or 0
	if _depth > 32 then return to_overwrite end
	for i, v in pairs(overwrite_with) do
		if v ~= to_overwrite[i] then

		if type(v) == "table" and type(to_overwrite[i]) == "table" then
			Overwrite(to_overwrite[i], v, _depth + 1)
		elseif type(v) == "table" then
			to_overwrite[i] = Overwrite({}, v, _depth + 1)
		else
			to_overwrite[i] = v
		end
		end
...
Library.Elements.Slider = function(self: Library, propertyTable: {})
	...
	Slider.Set = function(Value: number)
		...
		Slider.Value = tonumber(SF("%." .. tostring(Decimals) .. "f", Value))
		...
		Amount.Text = SF("%." .. tostring(Decimals) .. "f", Slider.Value) .. Slider.Suffix
...
Library.SetKeybindList = function(Enabled: boolean?)
	...
	local Panel = Add("Frame", {
		...
		BorderSizePixel = 0;
		Visible = Library.KeybindList.Enabled == false;
		ZIndex = 120;
		Active = true;
	})
